...
class AdminService {
  ...
  public getUserRating = async (userId: any) => {
    ...
    let followingCount = await userFollowerModel.countDocuments({ user_id: userId, accepted: true });
    let followerCount = await userFollowerModel.countDocuments({ follower_id: userId, accepted: true });
