...
class AuthService {
    ...
    async logout(userData, reqData) {
        ...
        if (reqData.device_token) {
            device_tokens_1.default.findOneAndUpdate({
    ...
    async deviceTokenAdd(userData, reqData) {
        const tokenExists = await device_tokens_1.default.findOne({
            user_id: userData._id,
            device_token: reqData.device_token,
        });
        if (tokenExists) {
            throw new HttpException_1.HttpException(409, 'Same token cannot be added');
        }
        device_tokens_1.default.create({
            user_id: userData._id,
            device_token: reqData.device_token,
        });
    }
    ...
    async followerRequest(userId, fullname, profilePhoto, reqData) {
        ...
        const message = acceptedState === constants_1.ACCOUNT_TYPE_CONST.PRIVATE
